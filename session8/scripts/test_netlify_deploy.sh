...
echo "get_site_id: $(curl -s -o /dev/null -w '%{http_code}' -H "Authorization: Bearer $PAT" https://api.netlify.com/api/v1/sites/23eb2bfe-1bd9-4d30-ac96-57564b9dee0f)"
echo "get_site_name: $(curl -s -o /dev/null -w '%{http_code}' -H "Authorization: Bearer $PAT" https://api.netlify.com/api/v1/sites/attention-evolution-session8)"

cd dist && zip -qr ../deploy.zip . && cd ..
echo "zip_deploy_id: $(curl -s -o /tmp/nd.json -w '%{http_code}' -H "Authorization: Bearer $PAT" -H "Content-Type: application/zip" --data-binary @deploy.zip https://api.netlify.com/api/v1/sites/23eb2bfe-1bd9-4d30-ac96-57564b9dee0f/deploys)"
