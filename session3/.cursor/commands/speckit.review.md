...
Before executing this command, **invoke** the Superpowers skills for phase `review` from merged `.specify/extensions/fleet/fleet-config.yml` → `superpowers.skills_by_phase_id.review` (see `.specify/memory/superpowers-fleet.md`). **Minimum:** `requesting-code-review` (structured, critical pre-implementation review). After this read-only review, for **very large** deltas only, you may **suggest** the **code-reviewer** subagent if listed under `superpowers.optional_subagents.review` — it **adds** a second pass; it does not replace this command’s `review.md` output.
