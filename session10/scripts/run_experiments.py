...
from truth_lab.float_repr import format_table, represent_value
from truth_lab.gradient_check import verify_gradient
from truth_lab.mfu import measure_mfu
from truth_lab.model import TinyGPT
from truth_lab.tensor_trace import format_trace_table, print_all_traces, trace_training_step
...
def run_tensor_trace(cfg: LabConfig, corpus: TinyCorpus) -> dict:
    ...
    text = print_all_traces(trace, grads)
    table = format_trace_table(trace)
    sample_decode = corpus.decode_tensor(x[0])
    return {
        "trace_text": text,
        "table": table,
        "sample_sentence": sample_decode,
...
def run_gradient_check(cfg: LabConfig, corpus: TinyCorpus) -> dict:
    ...
    x, y, m = x.to(device), y.to(device), m.to(device)
    result = verify_gradient(model, x, y, m)
    return {
...
def run_accumulation(cfg: LabConfig, corpus: TinyCorpus) -> dict:
    device = torch.device(cfg.device)
    # ponytail: 100 valid tokens need T>=100; use widened block only for this experiment
...
def run_grad_norm(cfg: LabConfig, corpus: TinyCorpus) -> dict:
    ...
    spike_step = max(range(len(norms)), key=lambda i: norms[i])
    return {
        "plot": str(PLOTS / "loss_and_grad_norm.png"),
        "grad_before_loss_event": event,
        "max_grad_norm_step": spike_step,
        "max_grad_norm": norms[spike_step],
        "final_loss": losses[-1],
...
def main() -> None:
    ...
    # Truth report verdicts
    gc = results["gradient_check"]
    gn = results["grad_norm"]
    results["truth_report"] = {
        "tensor_shapes": "PASS",
        "gradient": gc["verdict"],
        "accumulation": "PASS" if results["accumulation"]["max_loss_diff"] > 0 else "INVESTIGATE",
        "gradient_signal": "PASS" if gn["grad_before_loss_event"] else "INVESTIGATE",
        "mfu": "PASS",
        "precision": "PASS",
    }
