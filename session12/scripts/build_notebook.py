#!/usr/bin/env python3
"""Generate session12 notebook from simulator modules."""
...
NB_PATH = ROOT / "notebooks" / "session12_zero_32gpu_simulation.ipynb"
...
def reflection(observation: str, question: str, answer: str) -> list:
    return [
        md(f"**OBSERVATION:**\n\n{observation}"),
...
cells = [
    ...
    md("## 08. Baseline Data Parallel"),
    code("m_base = run_strategy('baseline_dp', cfg)\nprint(m_base)"),
    *reflection(
        ...
        "Why does every GPU consume approximately the same model-state memory?",
    ...
    md("## 09. ZeRO-1"),
    code("m_z1 = run_strategy('zero1', cfg)\nprint(m_z1)"),
    *reflection(
        ...
        "Which memory component stopped being fully replicated? Why?",
    ...
    md("## 10. ZeRO-2"),
    code("m_z2 = run_strategy('zero2', cfg)\nprint(m_z2)"),
    *reflection(
        ...
        "Why does sharding gradients save memory but not remove parameter replication?",
    ...
    md("## 11. ZeRO-3"),
    code("m_z3 = run_strategy('zero3', cfg)\nprint(m_z3)"),
    *reflection(
        ...
        "Why does memory decrease further? Where did additional communication come from?",
    ...
    md("## 12. Communication simulation"),
    code(
    ...
    md("## 14. Communication/compute overlap (conceptual)"),
    code(
    ...
    md("## 19. Concept → Evidence\n\n| Concept | Evidence in this notebook |\n|--------|---------------------------|\n| Data Parallelism | `m_base` peak memory |\n| All-reduce | baseline `communication_bytes` |\n| ZeRO-1 | optimizer shard ownership |\n| ZeRO-2 | reduce-scatter + grad shards |\n| ZeRO-3 | all-gather + param shards |\n| Overlap | bucket experiment rows + overlap cell |"),
    md(
        "# What I Understood\n\n"
        "The through-line for me is **ownership**: baseline DP replicates everything; each ZeRO stage shards one more tensor class until ZeRO-3 shards params, grads, and optimizer. "
        "Memory accounting follows those sets literally — not a single “16 bytes per param” guess. "
        "Communication is whatever collectives the strategy calls, with bytes and simulated time coming from `communication.py`, not from CUDA timers. "
        "See the seven-question table below and the final reflection for the trade-off story."
    ),
    md("## Seven core questions (concept → this lab)"),
    md(
        "**1. Why doesn't normal data parallelism reduce model-state memory per GPU?**  \n"
        "Because each rank still stores full weights, full gradients, and full optimizer state (`assign_replicated`). "
        "In my run, peak memory stayed ~16 MB per GPU from world size 1 through 32 on baseline — only aggregate cluster memory grows.\n\n"
        ...
        "ZeRO-3 cut steady per-GPU model-state memory to ~0.55 MB (aggregate accounting) but raised bytes moved and introduced a higher **peak** (~2.58 MB) during gathers. "
        "You buy headroom with sharding; you pay in collective traffic and transient buffers — still only a simulation, not a GPU benchmark."
    ),
    md("## Reflection questions (summary)"),
    *reflection(
        "Multi-node ZeRO-3 (`exp6`) reports `communication_time` **0.000384** s vs single-node assumption (`exp5`) **0.000154** s for the same **6,000,000** bytes. "
        "Baseline scaling: `total_step_time` goes from **0.624** s at world size 1 to **0.62464** s at 32 while `peak_memory_per_gpu` stays ~16 MB.",
        "Why can adding GPUs fail to provide proportional speedup?",
        "Compute work per step did not shrink when I added GPUs — the simulator still charges each rank the same `compute_work`. "
        "What changed is communication time and idle fraction (comm while waiting on slower inter-node links in exp6). "
        "So I get more aggregate batch parallelism in principle, but not a proportionally shorter step if comm or replication overhead dominates; here comm is small versus compute, so the curve is nearly flat.",
    ),
    md("## 22. Final observations\n\nAll metrics above come from `outputs/experiment_matrix.json` produced in-section (default `SimConfig`, 1M parameters, 32 ranks). Re-run `python scripts/run_all.py` after changing code or config."),
    md("## 23. Reproducibility\n\n```bash\ncd session12\npython3 -m venv .venv && source .venv/bin/activate\npip install -r requirements.txt\npython scripts/run_all.py\n```"),
    md("## Interactive summary tables"),
    code(
    ...
    md(
        ...
        "Building this lab forced me to stop thinking of ZeRO as a single “memory trick” and instead track **who owns each element** on each of the 32 virtual GPUs. "
        "Baseline DP made the redundancy obvious: every rank’s map covered all one million parameter elements, so peak memory barely moved when I scaled world size — only the cluster-wide total ballooned. "
        "ZeRO-1 chipped away at Adam state, ZeRO-2 added gradient shards and a reduce-scatter, and ZeRO-3 finally shard parameters — each stage changed the ownership sets and the collectives in `strategies/`, and the experiment JSON lined up with that (peak memory stepped down, ZeRO-3 comm bytes stepped up).\n\n"
        "The part that clicked late was **steady vs peak** on ZeRO-3: ~0.55 MB steady per GPU in the aggregate accounting versus ~2.58 MB peak when `temp_full_param_elements` models the all-gather window. "
        "That is the trade-off in one line — you do not get shard-sized memory and zero communication at the same time.\n\n"
        "I am treating all timings and bandwidths here as **simulation assumptions** from `communication.py`. "
        "They helped me reason about idle time and topology (exp5 vs exp6), but I would not present them as measured GPU performance — the value is the traceable chain from sharding rules to bytes moved to my interpretation."
    ),
]
