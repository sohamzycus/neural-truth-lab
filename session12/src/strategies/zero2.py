"""ZeRO Stage 2 — gradients + optimizer sharded."""

from __future__ import annotations

from ..communication import apply_comm_to_gpus, reduce_scatter
...
from ..virtual_gpu import assign_zero2_grad_opt_shard, make_cluster


def run_zero2_step(cfg: SimConfig, model: DemoModel | None = None) -> StepMetrics:
    model = model or DemoModel.build(cfg)
    gpus = make_cluster(cfg)
    total = cfg.parameter_count
    assign_zero2_grad_opt_shard(gpus, total, cfg.world_size)
    ...
    comm = reduce_scatter(cfg, ranks, grad_bytes)
    ...
    for gpu in gpus:
        ...
        bd = mem.breakdown(pending_comm_bytes=grad_bytes // cfg.world_size)
